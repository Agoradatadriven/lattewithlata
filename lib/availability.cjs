...
function check(settings, bookings, opts) {
  ...
  if (type === 'recording') {
    if (!admin && party > settings.recordingMaxParty) return { ok: false, field: 'party', message: `Recording night seats are limited to ${settings.recordingMaxParty} per reservation.` };
