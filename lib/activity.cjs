...
const what = (b) => (b.type === 'recording'
  ? `${b.party} ${b.party === 1 ? 'seat' : 'seats'} at the Thursday recording night on ${humanDate(b.date)}, doors ${humanTime(b.time)}`
...
function contactAcknowledgement(app, m) {
  return {
    ...
    text: `Hello ${m.name},\n\nThank you for writing to us. A person reads every message and we will reply within 2 working days.\n\n${signature(app)}`,
  };
}
